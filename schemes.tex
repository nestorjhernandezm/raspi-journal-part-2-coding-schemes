...
In \eqref{eq:recoded_packet}, $w_{i,j}$ is the coding coefficient used
...
$\textbf{V}$. Denoting ${(\textbf{W} \textbf{V})}_{i,j}$ as the element
...
{(\textbf{W} \textbf{V})}_{i,j} = \sum_{k=1}^{g} w_{i,k} v_{k,j}\ ,\ \forall i,j \in [1,g] \times [1,g]
\end{align}
