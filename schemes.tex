...
computed as packets are progressively received, in order to minimize the computation
delay for each step. The coding matrix is represented as follows:
%
...
for reducing the matrix by elementary row operations are carried each
a \ac{l.i.} packet is received. Once in reduced echelon form, packets
can be retrieved by doing a substitution starting from the lastest
coded packet. In this way, the amount of elementary operations at the end
is diminished.

\subsubsection{Recoding}
An inherent property of \ac{NC} is the possibility for intermediate nodes
in a network between a source and a destination, to create coded packets
even from coded packets sent from the source. In general, a recoded packet should be indistinguishable
