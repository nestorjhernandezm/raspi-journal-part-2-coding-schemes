...
regardless of the amount of times where recoding was performed. Similarly as
with the encoding operation, any decoder that collects a set of $g$ \ac{l.i.} recoded
packets with their respective coefficients, will be able to decode the data
as mentioned before in Section \ref{sssec:decoding}.
...
coding coefficients to zero when generating a coded packet. A way to
accomplish this, is to set a fixed ratio of non-zero coefficients
in the encoding vector of size $g$. We refer to this fixed ratio as the
average coding density $d$. Thus, for any coded packet $C_i$ with coding
coefficients $v_{ij},\ j \in [1,g]$, its coding density is defined as follows:
%
\begin{align}
...
\end{align}
%
From the density definition in \eqref{eq:coding_density}, it can be observed that
$0 \leq d \leq 1$. As $g$ increased, we obtain more granularity in the density.
Notice that the special case $d = 0$ has no practical purpose since it
implies just zero padded data. So, a practical range for the density excludes
the zero case, $0 < d \leq 1$. Moreover, it can be also observed that the
case of $d = 1$ reduces to the \ac{RLNC} coding scheme described in Section
\ref{ssec:RLNC}.

To achieve a desired average coding density in the coefficients, for each
of them, we utilize a set of Bernoulli random variables all with parameter $d$, i.e.
$\mathbb{B}_j \sim Bernoulli(d)\ ,\ \forall j \in [1,g]$. In this way, we
can represent a coded packet in \ac{SRLNC} as:
...
\begin{align} \label{eq:sparse_coded_packet}
    ...
    C_i  &= \bigoplus_{j=1}^{g} \mathbb{B}_j v_{ij} \otimes P_j ,\ \forall i \in [1,g] ,\ \
    d \in
    \begin{cases}
        {(0,0.5]} & \text{for } q = 2 \\
        {(0,1]} & \text{for } q > 2
...
\end{align}
%
In \eqref{eq:sparse_coded_packet}, we have specified a dependency on the field size
for practical density values. In case of employing $GF(2)$, the maximum plausible
density is restricted up to 0.5 since higher values incur in more frequent \ac{l.d.}
coded packets \textbf{Include reference here}. Reducing $d$ enables the encoder to
decrease the average number of packets manipulated to make a coded packet. This
reduces the complexity of the encoder since it needs to mix less packets. Moreover,
it also simplifies the decoder processing given that less nonzero coding coefficients
required to be operated on during the Gaussian elimination stage.

The drawback of this scheme is that coded packets from the encoder become more \ac{l.d.}
on each other as the density is reduced. This incurs in transmission overhead
since more coded packets are required to be sent to overcome the reception of redundant
packets. Also, particularly with this method, generating a coded packet may imply
that it could not contain any data at all. For example, we might find the case
where $\left[\mathbb{B}_1, \ldots, \mathbb{B}_g \right] = \mathbf{0}$. In that case,
the encoder discards the coded packet and tries to generate a new one, though this is
unlikely to occur in general.

Under the current method for setting the density, most coded packets that
are produced by the encoder are typically very sparse.
The reason being that here we control the \textit{average} amount
of nonzero coefficients, but not the exact value for each coded packet. Therefore,
we may have some cases where we have more or less packets combined. A second
implementation in Kodo considers doing something more clever.


As is shown in \eqref{eq:sparse_coded_packet2}, that implementation
proved faster for low densities \textbf{reference?}. It is best expressed as:
%
