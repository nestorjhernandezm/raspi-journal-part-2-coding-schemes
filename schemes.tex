...
of reducing the overall processing in both encoding and decoding. In what
follows, we describe the coding scheme with a basic method to produce sparse
coded packets and later we indicate a second one, giving reference to their
...
For this method, we first calculate the amount of coding coefficients
that we will use to combine. To do it so, a random number $\mathbb{N}$
of the original packets is used to produce a coded packet. For our case,
$\mathbb{N}$ is binomially distributed with parameters $g$ for the number
of trials and $d$ for its success probability, e.g. $\mathbb{N} \sim
Binomial(g,d)$. However, when sampling from this distribution, the case
...
more often than in the original distribution, but for the
considered density range in this method it is depreciable
\textbf{Same reference here}. Then, once the distribution number of packets
to mix as been defined, we create a subset $\mathcal{C}$ of size $k$


a coded packetas is shown in
\eqref{eq:sparse_coded_packet2}, that implementation
