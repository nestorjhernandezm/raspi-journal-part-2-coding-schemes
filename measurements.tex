...
process, if any. The number of changes is $C = \#regions - 1$. With
this definitions, we first evaluate \ac{TSNC} with no changes, e.g.
$C = 0$ and the two amounts for dependent packets mentioned earlier.
This means that we tolerate at most 8 or 16 packets regardless of the
generation. The goal with setting first \ac{TSNC} with these values is to
observe first the sole effect of the budget in the code since $C = 0$
implies no density changes during the whole process.
