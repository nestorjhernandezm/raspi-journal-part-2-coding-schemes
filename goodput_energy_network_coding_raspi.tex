...
by a large number of devices, there is a need in evaluating large-scale
deployments of distributed data-processing systems in a quick and manageable
fashion. In this sense, the Raspberry Pi has been a popular choice
as a representative device for a network node due to its reduced size,
processing capabilities, low cost and use of different open
source operating systems. Also, new paradigms such as network coding have
appeared for fast and reliable data processing in networking and storage
systems which overcome the limitations of state-of-the-art routing
techniques. Therefore, in this work we provide an in-depth performance
evaluation for \ac{RLNC} based schemes for the Raspberry Pi 1 and 2 models
by showing the processing speed for the encoding and decoding operations and
its energy consumption when carrying them. Our results show that processing
speeds of more than 80 Mbps and 800 Mbps are attainable for several
scenarios for the Raspberry Pi 1 and 2, respectively. Also analogously,
we show that the processing energy per bit for network coding is below 1
nJ and even an order of magnitude lower in the given scenarios.}
