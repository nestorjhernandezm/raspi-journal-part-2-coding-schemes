...
\abstract{The use of the Raspberry Pi as a low-complexity processing
node in large-scale networks permits a fast deployment and testing
of hundreds (or even thousands) of devices in distributed systems at a
fraction of the cost necessary in previous years. The underlying reason
is that now it is possible to run an standard open source \ac{OS}
with a credit card-sized single board computer on the processing device.
Also, new paradigms such as network coding have appeared for fast and
reliable data processing in networking and storage systems which overcome
the limitations of state-of-the-art routing techniques. Therefore, in this
work we provide an in-depth performance evaluation for network coding,
a new paradigm in networking and storage, and present the processing
